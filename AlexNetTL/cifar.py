...
def __unflaten_image__(img_flat):
    img_R = img_flat[0:1024].reshape((32, 32))
    img_G = img_flat[1024:2048].reshape((32, 32))
    img_B = img_flat[2048:3072].reshape((32, 32))
    img = np.dstack((img_R, img_G, img_B))
    return img/255 # normalized image
...
def __extract_reshape_file__(fname):
    ...
    for image, label in zip(images, labels):
        res.append((__unflaten_image__(image), label))
    return res
...
class Cifar(object):
    ...
    def next_batch(self):
        if self.__batch_num__ <= len(self.batches):
            ...
            self.__batch_num__ += self.__batch_num__
        else:
            res = []
        return res
    ...
    def create_resized_batches(self, new_size=(224,224), dim=1000):
        self.resized_batches = [helper.transform_to_input_output_and_pad(batch,
                                new_size=new_size, dim=dim) for batch in tqdm(self.batches, desc="Reshaping")]
        self.create_resized_test_set(new_size=new_size, dim=dim)
