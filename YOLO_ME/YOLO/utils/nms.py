...
def overlap_c(x1, w1 , x2 , w2):  # OVERLAP
    l1 = x1 - w1 /2.
    l2 = x2 - w2 /2.
    left = max(l1,l2)
    r1 = x1 + w1 /2.
...
def box_intersection_c(ax, ay, aw, ah, bx, by, bw, bh): # BOX_INTERSECTION
...
def box_union_c(ax, ay, aw, ah, bx, by, bw, bh): # BOX_UNION
    i = box_intersection_c(ax, ay, aw, ah, bx, by, bw, bh)
    u = aw * ah + bw * bh -i
    return u


def box_iou_c(ax, ay, aw, ah, bx, by, bw, bh):  # BOX_IOU
    return box_intersection_c(ax, ay, aw, ah, bx, by, bw, bh) / box_union_c(ax, ay, aw, ah, bx, by, bw, bh);


def NMS(final_probs ,  final_bbox): # BOX_NMS
    boxes = list()
    indices = set()
