...
def generate_output(input_files):
    ...
    file_dir = "C:\\Users\\mpole\\Dataset\\ImageAnnotations\\"
    file_name = os.path.join(file_dir + "ImageAnnotations.csv")
    target_map = {}
    with open(file_name, 'r') as f:
        for line in f:
            line = line[:-1] # strip newline at the enf
            target_id, target_name = line.split(',')
            target_map[int(target_id)] = target_name
        target_map[0] = 'background'  # class ID 0 reserved for background class
